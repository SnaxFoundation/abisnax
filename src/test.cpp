...
template <typename T>
T check_context(abisnax_context* context, T value) {
    if (!value)
        throw std::runtime_error(abisnax_get_error(context));
    return value;
}

void run_check_type(abisnax_context* context, uint64_t contract, const char* type, const char* data,
                    const char* expected = nullptr, bool check_ordered = true) {
    if (!expected)
        expected = data;
    // printf("%s %s\n", type, data);
    check_context(context, abisnax_json_to_bin_reorderable(context, contract, type, data));
    std::string reorderable_hex = check_context(context, abisnax_get_bin_hex(context));
    if (check_ordered) {
        check_context(context, abisnax_json_to_bin(context, contract, type, data));
        std::string ordered_hex = check_context(context, abisnax_get_bin_hex(context));
        if (reorderable_hex != ordered_hex)
            throw std::runtime_error("mismatch between reorderable_hex, ordered_hex");
    }
    // printf("%s\n", reorderable_hex.c_str());
    std::string result = check_context(context, abisnax_hex_to_json(context, contract, type, reorderable_hex.c_str()));
    // printf("%s\n", result.c_str());
...
template <typename F>
void check_error(abisnax_context* context, const std::string& s, F f) {
    check_except(s, [&] { check_context(context, f()); });
}

void check_types() {
    auto context = check(abisnax_create());
    auto token = check_context(context, abisnax_string_to_name(context, "snax.token"));
    auto testAbiName = check_context(context, abisnax_string_to_name(context, "test.abi"));
    auto testHexAbiName = check_context(context, abisnax_string_to_name(context, "test.hex"));
    check_context(context, abisnax_set_abi(context, 0, transactionAbi));
    check_context(context, abisnax_set_abi_hex(context, token, tokenHexAbi));
    check_context(context, abisnax_set_abi(context, testAbiName, testAbi));
    check_context(context, abisnax_set_abi_hex(context, testHexAbiName, testHexAbi));

    int next_id = 0;
    auto write_corpus = [&](bool abi_is_bin, uint8_t operation, uint64_t contract, abisnax::input_buffer abi,
                            abisnax::input_buffer type, abisnax::input_buffer data) {
    ...
    auto check_type = [&](abisnax_context* context, uint64_t contract, const char* type, const char* data,
                          const char* expected = nullptr, bool check_ordered = true) {
        if (!generate_corpus)
        ...
        check_context(context, abisnax_json_to_bin_reorderable(context, contract, type, data));
        std::string hex = check_context(context, abisnax_get_bin_hex(context));
    ...
    check_error(context, "no data", [&] { return abisnax_set_abi_hex(context, 8, ""); });
    check_error(context, "unsupported abi version", [&] { return abisnax_set_abi_hex(context, 8, "00"); });
    check_error(context, "unsupported abi version",
                [&] { return abisnax_set_abi_hex(context, 8, string_to_hex("snax::abi/9.0").c_str()); });
    check_error(context, "read past end",
                [&] { return abisnax_set_abi_hex(context, 8, string_to_hex("snax::abi/1.0").c_str()); });
    check_error(context, "read past end",
                [&] { return abisnax_set_abi_hex(context, 8, string_to_hex("snax::abi/1.1").c_str()); });

    check_error(context, "unsupported abi version",
                [&] { return abisnax_set_abi(context, 8, R"({"version":"snax::abi/9.0"})"); });
    abisnax_set_abi(context, 8, R"({"version":"snax::abi/1.0"})");
    abisnax_set_abi(context, 8, R"({"version":"snax::abi/1.1"})");

    check_type(context, 0, "bool", R"(true)");
    check_type(context, 0, "bool", R"(false)");
    check_error(context, "read past end", [&] { return abisnax_hex_to_json(context, 0, "bool", ""); });
    ...
    check_error(context, "number is out of range", [&] { return abisnax_json_to_bin(context, 0, "int8", "128"); });
    check_error(context, "number is out of range", [&] { return abisnax_json_to_bin(context, 0, "int8", "-129"); });
    check_error(context, "expected non-negative number", [&] { return abisnax_json_to_bin(context, 0, "uint8", "-1"); });
    check_error(context, "number is out of range", [&] { return abisnax_json_to_bin(context, 0, "uint8", "256"); });
    ...
    check_error(context, "read past end", [&] { return abisnax_hex_to_json(context, 0, "int16", "01"); });
    check_type(context, 0, "uint16", R"(0)");
    check_type(context, 0, "uint16", R"(65535)");
    check_error(context, "number is out of range", [&] { return abisnax_json_to_bin(context, 0, "int16", "32768"); });
    check_error(context, "number is out of range", [&] { return abisnax_json_to_bin(context, 0, "int16", "-32769"); });
    check_error(context, "expected non-negative number",
                [&] { return abisnax_json_to_bin(context, 0, "uint16", "-1"); });
    check_error(context, "number is out of range", [&] { return abisnax_json_to_bin(context, 0, "uint16", "655356"); });
    ...
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "int32", "2147483648"); });
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "int32", "-2147483649"); });
    check_error(context, "expected non-negative number",
                [&] { return abisnax_json_to_bin(context, 0, "uint32", "-1"); });
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "uint32", "4294967296"); });
    ...
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "int128", "170141183460469231731687303715884105728"); });
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "int128", "-170141183460469231731687303715884105729"); });
    check_error(context, "expected string containing int128",
                [&] { return abisnax_json_to_bin(context, 0, "int128", "true"); });
    check_error(context, "invalid number", [&] { return abisnax_json_to_bin(context, 0, "uint128", "-1"); });
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "uint128", "340282366920938463463374607431768211456"); });
    check_error(context, "expected string containing uint128",
                [&] { return abisnax_json_to_bin(context, 0, "uint128", "true"); });
    ...
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "varint32", "2147483648"); });
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "varint32", "-2147483649"); });
    check_error(context, "expected non-negative number",
                [&] { return abisnax_json_to_bin(context, 0, "varuint32", "-1"); });
    check_error(context, "number is out of range",
                [&] { return abisnax_json_to_bin(context, 0, "varuint32", "4294967296"); });
    ...
    check_error(context, "expected string containing time_point_sec",
                [&] { return abisnax_json_to_bin(context, 0, "time_point_sec", "true"); });
    ...
    check_error(context, "expected string containing time_point",
                [&] { return abisnax_json_to_bin(context, 0, "time_point", "true"); });
    ...
    check_error(context, "expected string containing name",
                [&] { return abisnax_json_to_bin(context, 0, "name", "true"); });
    check_type(context, 0, "bytes", R"("")");
    check_type(context, 0, "bytes", R"("00")");
    check_type(context, 0, "bytes", R"("AABBCCDDEEFF00010203040506070809")");
    check_error(context, "odd number of hex digits", [&] { return abisnax_json_to_bin(context, 0, "bytes", R"("0")"); });
    check_error(context, "expected hex string", [&] { return abisnax_json_to_bin(context, 0, "bytes", R"("yz")"); });
    check_error(context, "expected string containing hex digits",
                [&] { return abisnax_json_to_bin(context, 0, "bytes", R"(true)"); });
    check_error(context, "invalid bytes size", [&] { return abisnax_hex_to_json(context, 0, "bytes", "01"); });
    ...
    check_error(context, "invalid string size", [&] { return abisnax_hex_to_json(context, 0, "string", "01"); });
    ...
    check_error(context, "expected hex string",
                [&] { return abisnax_json_to_bin(context, 0, "checksum256", R"("yz")"); });
    check_error(context, "expected string containing hex",
                [&] { return abisnax_json_to_bin(context, 0, "checksum256", R"(true)"); });
    check_error(context, "hex string has incorrect length",
                [&] { return abisnax_json_to_bin(context, 0, "checksum256", R"("a0")"); });
    check_type(context, 0, "public_key", R"("SNAX1111111111111111111111111111111114T1Anm")",
               R"("PUB_K1_11111111111111111111111111111111149Mr2R")");
    check_type(context, 0, "public_key", R"("SNAX11111111111111111111111115qCHTcgbQwptSz99m")",
               R"("PUB_K1_11111111111111111111111115qCHTcgbQwpvP72Uq")");
    check_type(context, 0, "public_key", R"("SNAX111111111111111114ZrjxJnU1LA5xSyrWMNuXTrYSJ57")",
               R"("PUB_K1_111111111111111114ZrjxJnU1LA5xSyrWMNuXTrVub2r")");
    check_type(context, 0, "public_key", R"("SNAX1111111113diW7pnisfdBvHTXP7wvW5k5Ky1e5DVuF23dosU")",
               R"("PUB_K1_1111111113diW7pnisfdBvHTXP7wvW5k5Ky1e5DVuF4PizpM")");
    check_type(context, 0, "public_key", R"("SNAX11DsZ6Lyr1aXpm9aBqqgV4iFJpNbSw5eE9LLTwNAxqjJgmjgbT")",
               R"("PUB_K1_11DsZ6Lyr1aXpm9aBqqgV4iFJpNbSw5eE9LLTwNAxqjJgXSdB8")");
    check_type(context, 0, "public_key", R"("SNAX12wkBET2rRgE8pahuaczxKbmv7ciehqsne57F9gtzf1PVYNMRa2")",
               R"("PUB_K1_12wkBET2rRgE8pahuaczxKbmv7ciehqsne57F9gtzf1PVb7Rf7o")");
    check_type(context, 0, "public_key", R"("SNAX1yp8ebBuKZ13orqUrZsGsP49e6K3ThVK1nLutxSyU5j9SaXz9a")",
               R"("PUB_K1_1yp8ebBuKZ13orqUrZsGsP49e6K3ThVK1nLutxSyU5j9Tx1r96")");
    check_type(context, 0, "public_key", R"("SNAX9adaAMuB9v8yX1mZ5PtoB6VFSCeqRGjASd8ZTM6VUkiHL7mue4K")",
               R"("PUB_K1_9adaAMuB9v8yX1mZ5PtoB6VFSCeqRGjASd8ZTM6VUkiHLB5XEdw")");
    check_type(context, 0, "public_key", R"("SNAX69X3383RzBZj41k73CSjUNXM5MYGpnDxyPnWUKPEtYQmTBWz4D")",
               R"("PUB_K1_69X3383RzBZj41k73CSjUNXM5MYGpnDxyPnWUKPEtYQmVzqTY7")");
    check_type(context, 0, "public_key", R"("SNAX7yBtksm8Kkg85r4in4uCbfN77uRwe82apM8jjbhFVDgEgz3w8S")",
               R"("PUB_K1_7yBtksm8Kkg85r4in4uCbfN77uRwe82apM8jjbhFVDgEcarGb8")");
    check_type(context, 0, "public_key", R"("SNAX7WnhaKwHpbSidYuh2DF1qAExTRUtPEdZCaZqt75cKcixuQUtdA")",
               R"("PUB_K1_7WnhaKwHpbSidYuh2DF1qAExTRUtPEdZCaZqt75cKcixtU7gEn")");
    check_type(context, 0, "public_key", R"("SNAX7Bn1YDeZ18w2N9DU4KAJxZDt6hk3L7eUwFRAc1hb5bp6xJwxNV")",
    ...
    check_error(context, "expected string containing public_key",
                [&] { return abisnax_json_to_bin(context, 0, "public_key", "true"); });
    check_error(context, "unrecognized public key format",
                [&] { return abisnax_json_to_bin(context, 0, "public_key", R"("foo")"); });
    ...
    check_error(context, "expected string containing private_key",
                [&] { return abisnax_json_to_bin(context, 0, "private_key", "true"); });
    check_error(context, "unrecognized private key format",
                [&] { return abisnax_json_to_bin(context, 0, "private_key", R"("foo")"); });
    check_type(
        context, 0, "signature",
    ...
    check_type(context, 0, "asset", R"("1.2345 SNAX")");
    check_type(context, 0, "asset", R"("-1.2345 SNAX")");
    check_error(context, "expected string containing asset",
                [&] { return abisnax_json_to_bin(context, 0, "asset", "null"); });
    ...
    check_type(context, token, "transfer",
               R"({"from":"useraaaaaaaa","to":"useraaaaaaab","quantity":"0.0001 SNAX","memo":"test memo"})");
    check_type(
        context, 0, "transaction",
        R"({"expiration":"2009-02-13T23:31:31.000","ref_block_num":1234,"ref_block_prefix":5678,"max_net_usage_words":0,"max_cpu_usage_ms":0,"delay_sec":0,"context_free_actions":[],"actions":[{"account":"snax.token","name":"transfer","authorization":[{"actor":"useraaaaaaaa","permission":"active"}],"data":"608C31C6187315D6708C31C6187315D60100000000000000045359530000000000"}],"transaction_extensions":[]})");

    check_type( //
        context, token, "transfer",
        R"({"to":"useraaaaaaab","memo":"test memo","from":"useraaaaaaaa","quantity":"0.0001 SNAX"})",
        R"({"from":"useraaaaaaaa","to":"useraaaaaaab","quantity":"0.0001 SNAX","memo":"test memo"})", false);
    check_type(
        context, 0, "transaction",
        R"({"ref_block_num":1234,"ref_block_prefix":5678,"expiration":"2009-02-13T23:31:31.000","max_net_usage_words":0,"max_cpu_usage_ms":0,"delay_sec":0,"context_free_actions":[],"actions":[{"account":"snax.token","name":"transfer","authorization":[{"actor":"useraaaaaaaa","permission":"active"}],"data":"608C31C6187315D6708C31C6187315D60100000000000000045359530000000000"}],"transaction_extensions":[]})",
        R"({"expiration":"2009-02-13T23:31:31.000","ref_block_num":1234,"ref_block_prefix":5678,"max_net_usage_words":0,"max_cpu_usage_ms":0,"delay_sec":0,"context_free_actions":[],"actions":[{"account":"snax.token","name":"transfer","authorization":[{"actor":"useraaaaaaaa","permission":"active"}],"data":"608C31C6187315D6708C31C6187315D60100000000000000045359530000000000"}],"transaction_extensions":[]})",
        false);

    check_error(context, "recursion limit reached", [&] {
        return abisnax_json_to_bin_reorderable(
            context, 0, "int8",
            "[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[["
            "[[[[[[[[[[[[[[[[[[[[[[[]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]"
            "]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]");
    });
    check_error(context, "json parse error", [&] { return abisnax_json_to_bin_reorderable(context, 0, "int8", "1,2"); });

    check_error(context, "optional (?) and array ([]) don't support nesting",
                [&] { return abisnax_json_to_bin(context, 0, "int8?[]", ""); });
    check_error(context, "optional (?) and array ([]) don't support nesting",
                [&] { return abisnax_json_to_bin(context, 0, "int8[]?", ""); });
    ...
    check_error(context, "binary extensions ($) may not contain binary extensions ($)",
                [&] { return abisnax_json_to_bin(context, 0, "int8$$", ""); });
    check_error(context, "unknown type \"fee\"", [&] { return abisnax_json_to_bin(context, 0, "fee", ""); });

    check_error(context, "abi has a type with a missing name", [&] {
        return abisnax_set_abi( //
            context, 0,
            R"({"version":"snax::abi/1.1","types":[{"new_type_name":"","type":"int8"}]})");
    });
    check_error(context, "can't use extensions ($) within typedefs", [&] {
        return abisnax_set_abi( //
            context, 0,
            R"({"version":"snax::abi/1.1","types":[{"new_type_name":"a","type":"int8$"}]})");
    });
    check_error(context, "abi redefines type \"a\"", [&] {
        return abisnax_set_abi(
            context, 0,
            R"({"version":"snax::abi/1.1","types":[{"new_type_name":"a","type":"int8"},{"new_type_name":"a","type":"int8"}]})");
    });

    check_error(context, "expected object", [&] { return abisnax_json_to_bin(context, testAbiName, "s4", "null"); });
    check_error(context, "expected object", [&] { return abisnax_json_to_bin(context, testAbiName, "s4", "[]"); });
    check_error(context, R"(s4: expected field "a1")",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s4", R"({"foo":7})"); });
    check_error(context, R"(s4.a1: expected number or boolean)",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s4", R"({"a1":[]})"); });
    check_error(context, R"(expected variant: ["type", value])",
                [&] { return abisnax_json_to_bin(context, testAbiName, "v1", "null"); });
    check_error(context, R"(<variant>: expected variant: ["type", value])",
                [&] { return abisnax_json_to_bin(context, testAbiName, "v1", "[]"); });
    check_error(context, R"(<variant>: type is not valid for this variant)",
                [&] { return abisnax_json_to_bin(context, testAbiName, "v1", R"(["x"])"); });
    check_error(context, R"(<variant>: expected variant: ["type", value])",
                [&] { return abisnax_json_to_bin(context, testAbiName, "v1", R"(["int8",7,5])"); });
    check_error(context, R"(s5: expected field "x1")",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({})"); });
    check_error(context, R"(s5: expected field "x2")",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":5})"); });
    check_error(context, R"(s5: expected field "x3")",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":5,"x2":7})"); });
    check_error(context, R"(s5.x1: expected number or boolean)",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":null})"); });
    check_error(context, R"(s5.x2: expected number or boolean)",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":null})"); });
    check_error(context, R"(s5: expected field "x3")",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10})"); });
    check_error(context, R"(s5.x3: expected object)",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":null)"); });
    check_error(context, R"(s5.x3: expected field "c1")",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{})"); });
    check_error(context, R"(s5.x3: expected field "c2")",
                [&] { return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4})"); });
    check_error(context, R"(s5.x3.c2: expected array)", [&] {
        return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4,"c2":{}})");
    });
    check_error(context, R"(s5.x3.c2[0]: expected object)", [&] {
        return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4,"c2":[7])");
    });
    check_error(context, R"(s5.x3.c2[0]: expected field "x1")", [&] {
        return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4,"c2":[{}])");
    });
    check_error(context, R"(s5.x3.c2[0].x1: expected number or boolean)", [&] {
        return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4,"c2":[{"x1":null}]}} )");
    });
    check_error(context, R"(s5.x3.c2[0]: expected field "x2")", [&] {
        return abisnax_json_to_bin(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4,"c2":[{"x1":7}]}} )");
    });
    check_error(context, R"(s5.x3.c2[0]: expected field "x2")", [&] {
        return abisnax_json_to_bin(context, testAbiName, "s5",
    ...
    check_error(context, "expected object",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s4", "null"); });
    check_error(context, "expected object",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s4", "[]"); });
    check_error(context, R"(s4.a1: expected number or boolean)",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s4", R"({"a1":[]})"); });
    check_error(context, R"(expected variant: ["type", value])",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "v1", "null"); });
    check_error(context, R"(expected variant: ["type", value])",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "v1", "[]"); });
    check_error(context, R"(expected variant: ["type", value])",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "v1", R"(["x",7,5])"); });
    check_error(context, R"(<variant>: type is not valid for this variant)",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "v1", R"(["x",7])"); });
    check_error(context, R"(expected variant: ["type", value])",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "v1", R"(["int8",7,5])"); });
    check_error(context, R"(s5: expected field "x1")",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({})"); });
    check_error(context, R"(s5: expected field "x2")",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":5})"); });
    check_error(context, R"(s5: expected field "x3")",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":5,"x2":7})"); });
    check_error(context, R"(s5.x1: expected number or boolean)",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":null})"); });
    check_error(context, R"(s5.x2: expected number or boolean)",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":9,"x2":null})"); });
    check_error(context, R"(s5: expected field "x3")",
                [&] { return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":9,"x2":10})"); });
    check_error(context, R"(s5.x3: expected object)", [&] {
        return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":null})");
    });
    check_error(context, R"(s5.x3: expected field "c1")", [&] {
        return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{}})");
    });
    check_error(context, R"(s5.x3: expected field "c2")", [&] {
        return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4}})");
    });
    check_error(context, R"(s5.x3.c2: expected array)", [&] {
        return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4,"c2":{}}})");
    });
    check_error(context, R"(s5.x3.c2[0]: expected object)", [&] {
        return abisnax_json_to_bin_reorderable(context, testAbiName, "s5", R"({"x1":9,"x2":10,"x3":{"c1":4,"c2":[7]}})");
    });
    check_error(context, R"(s5.x3.c2[0]: expected field "x1")", [&] {
        return abisnax_json_to_bin_reorderable(context, testAbiName, "s5",
