...
#include "abisnax_ripemd160.hpp"

#define ABISNAX_NODISCARD [[nodiscard]]

namespace abisnax {

template <typename State>
ABISNAX_NODISCARD bool set_error(State& state, std::string error) {
    state.error = std::move(error);
    return false;
}

ABISNAX_NODISCARD inline bool set_error(std::string& state, std::string error) {
...
template <auto size>
ABISNAX_NODISCARD inline bool decimal_to_binary(std::array<uint8_t, size>& result, std::string& error,
                                               std::string_view s) {
...
template <typename T>
ABISNAX_NODISCARD inline auto decimal_to_binary(T& result, std::string& error, std::string_view s)
...
template <auto size>
ABISNAX_NODISCARD inline bool base58_to_binary(std::array<uint8_t, size>& result, std::string& error,
                                              std::string_view s) {
    memset(result.begin(), 0, result.size());
    for (auto& src_digit : s) {
...
ABISNAX_NODISCARD inline bool digest_message_ripemd160(std::array<unsigned char, 20>& digest, std::string& error,
                                                      const unsigned char* message, size_t message_len) {
    abisnax_ripemd160::ripemd160_state self;
    abisnax_ripemd160::ripemd160_init(&self);
    abisnax_ripemd160::ripemd160_update(&self, message, message_len);
    if (!abisnax_ripemd160::ripemd160_digest(&self, digest.data()))
        return set_error(error, "ripemd failed");
    return true;
}

template <size_t size, int suffix_size>
ABISNAX_NODISCARD inline bool digest_suffix_ripemd160(std::array<unsigned char, 20>& digest, std::string& error,
                                                     const std::array<uint8_t, size>& data,
                                                     const char (&suffix)[suffix_size]) {
    abisnax_ripemd160::ripemd160_state self;
    abisnax_ripemd160::ripemd160_init(&self);
    abisnax_ripemd160::ripemd160_update(&self, data.data(), data.size());
    abisnax_ripemd160::ripemd160_update(&self, (uint8_t*)suffix, suffix_size - 1);
    if (!abisnax_ripemd160::ripemd160_digest(&self, digest.data()))
        return set_error(error, "ripemd failed");
    return true;
}
...
ABISNAX_NODISCARD bool string_to_key(Key& result, std::string& error, std::string_view s, key_type type,
                                    const char (&suffix)[suffix_size]) {
...
template <typename Key, int suffix_size>
ABISNAX_NODISCARD bool key_to_string(std::string& dest, std::string& error, const Key& key,
...
ABISNAX_NODISCARD inline bool string_to_public_key(public_key& dest, std::string& error, std::string_view s) {
...
ABISNAX_NODISCARD inline bool public_key_to_string(std::string& dest, std::string& error, const public_key& key) {
...
ABISNAX_NODISCARD inline bool string_to_private_key(private_key& dest, std::string& error, std::string_view s) {
...
ABISNAX_NODISCARD inline bool private_key_to_string(std::string& dest, std::string& error,
...
ABISNAX_NODISCARD inline bool string_to_signature(signature& dest, std::string& error, std::string_view s) {
...
ABISNAX_NODISCARD inline bool signature_to_string(std::string& dest, std::string& error, const signature& signature) {
